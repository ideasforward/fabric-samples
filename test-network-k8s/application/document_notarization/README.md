...
See [src/server.ts](src/server.ts) for a description of the sample code structure, and [src/config.ts](src/config/config.ts) for details of configuring the sample using environment variables. 
